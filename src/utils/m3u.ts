...
const REG_EXTINF = /^#EXTINF:((?:[0-9]*[.])?[0-9]+?)([^0-9].*?)?(?:,(.*?))?$/
const REG_EXT_X = /^#EXT-X-(.*?):(.*?)$/
  ...
  for (const line of lines) {
    if (line.startsWith('#')) {
      if (line.startsWith('#EXTINF')) {
        const matchTrackInfo = line.match(REG_EXTINF)
        if (!matchTrackInfo) {
        ...
        if (line.startsWith('#EXT-X-')) {
          const matchKeyValue = line.match(REG_EXT_X)
          if (!matchKeyValue) {
          ...
          if (key === 'VERSION') {
            ...
            extension.programeDateTime = new Date(value)
          } else {
